...
export type IncrementInstruction<
  TProgram extends string = typeof CRUD_PROGRAM_ADDRESS,
  TAccountCrud extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountCrud extends string
        ? WritableAccount<TAccountCrud>
        : TAccountCrud,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountCrud extends string = string> = {
  crud: Address<TAccountCrud>;
};

export function getIncrementInstruction<
  TAccountCrud extends string,
  TProgramAddress extends Address = typeof CRUD_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountCrud>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountCrud> {
  // Program address.
  const programAddress = config?.programAddress ?? CRUD_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    crud: { value: input.crud ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.crud)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountCrud>;
